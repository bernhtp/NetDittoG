...
void _stdcall
	DisplayPathOffset(
		wchar_t const	  * p_path		// in -target path
	)
{
	static short				len = -1;	// persistent original target path

	if ( len < 0 )						// first time
		len = (short)wcslen(p_path);		
	VtxtDispStrN(p_path + len, A_IBKG, 78, PATHROW, 1);
}
...
void _stdcall
   DisplayTime()
{
	__int64 static				tStart = 0,  // initial value
								tLast;
	__int64						tNow = GetTickCount64(),
								tDiff;
	DWORD						nWrite;
	wchar_t						temp[3][8];
	...
	tDiff = tNow - tStart;
	ElapsedTimeStr((DWORD)(tDiff / 1000), temp[0]);
	...
	else
	{
		if ( tNow != tLast )
		{
			...
			_swprintf(temp[1], L"%7d", (int)((gOptions.bWritten - bLastWritten) / (tNow - tLast)));
